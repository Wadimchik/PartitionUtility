DROP PROCEDURE IF EXISTS schema_name.table_name_create_new_partitions_size_years;

CREATE PROCEDURE schema_name.table_name_create_new_partitions_size_years(p_schema varchar(64), p_table varchar(64), p_two_days_to_keep int)
   LANGUAGE SQL
   NOT DETERMINISTIC
...
BEGIN  
   ...
   AND PARTITION_NAME != 'p_future';
    ...
    OPEN partitionsCur;
      read_loop: LOOP
        ...
        END IF;
         ...
         SET partitionsCur_partition_datetime = STR_TO_DATE(REPLACE(CONCAT(partitionsCur_partition_name, '01'), 'p', ''), '%Y%m%D');
    ...
    ELSE
        SET current_partition_datetime = DATE_ADD(curdate(), INTERVAL partition_interval YEAR);
   ...
   WHILE @days_ahead <= p_two_days_to_keep * partition_interval DO
      ...
      SET @q = 'SELECT DATE_ADD(?, INTERVAL ? YEAR) INTO @day_to_add';
      PREPARE st FROM @q;
      ...
      SET @q = 'SELECT DATE_FORMAT(@day_to_add, ''%Y%m'') INTO @formatted_day_to_add';
      PREPARE st FROM @q;
      EXECUTE st;
      DEALLOCATE PREPARE st;
      ...
      read_loop: LOOP
         ...
         IF done AND @first THEN
            ...
            SET @q = 'SELECT DATE_FORMAT(@day_to_add, ''%Y-%m-01 00:00:00'') INTO @day_to_add';
            PREPARE st FROM @q;
            EXECUTE st;
            DEALLOCATE PREPARE st; 
