DROP PROCEDURE IF EXISTS schema_name.table_name_drop_old_partitions_size_years;

CREATE PROCEDURE schema_name.table_name_drop_old_partitions_size_years(p_schema varchar(64), p_table varchar(64), p_years_to_keep int)
   LANGUAGE SQL
   NOT DETERMINISTIC
...
BEGIN  
   ...
   SET @date = CURDATE();
   SET @years_to_keep = p_years_to_keep * sectorSize;   
   SET @q = 'SELECT DATE_SUB(?, INTERVAL ? YEAR) INTO @last_year_to_keep';
   PREPARE st FROM @q;
   EXECUTE st USING @date, @years_to_keep;
   DEALLOCATE PREPARE st;
   
   SET @q = 'SELECT DATE_FORMAT(@last_year_to_keep, ''%Y%m'') INTO @formatted_last_year_to_keep';
   PREPARE st FROM @q;
   EXECUTE st;
   DEALLOCATE PREPARE st;
   
   SET @q = 'SELECT CONCAT(''p'', @formatted_last_year_to_keep) INTO @last_partition_name_to_keep';
   PREPARE st FROM @q;
   EXECUTE st;
   DEALLOCATE PREPARE st;
